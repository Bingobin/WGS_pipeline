...
echo "BWA \${PREFIX} \$i starts at `date`" &&\ bwa mem -t 20 -M -R "\@RG\\tID:\$i\\tLB:\$LAB\\tSM:\$SM\\tPL:ILLUMINA" -Y \$REF \$file1 \$file2 | samtools view -Sb -o \${PREFIX}_\${i}.bam &&\ echo "BWA \${PREFIX} \$i DONE at `date`"
...
\$JAVA -Xmx70g -jar \$PICARD MergeSamFiles SO=coordinate \$MERGE_FILE   O=\${PREFIX}.sorted.merged.bam TMP_DIR=\$TMP &&\\
echo "Mission  sort completes at `date`" &&\\

##########################Step03: Picard Reorder##########################################

echo "Mission reorder at `date`" &&\\
\$JAVA -Xmx70g -jar \$PICARD ReorderSam  I=\${PREFIX}.sorted.merged.bam  O=\${PREFIX}.sorted.merged.reorder.bam R=\$REF VALIDATION_STRINGENCY=LENIENT TMP_DIR=\$TMP && rm \${PREFIX}.sorted.merged.bam &&\\
...
\$JAVA -Djava.io.tmpdir=\$TMP -Xmx70g -jar \$GATK -T BaseRecalibrator -R \$REF -I \${PREFIX}.sorted.merged.reorder.rmdup.realign.bam -knownSites \$GATK_ann/dbsnp_146.hg38.vcf  -knownSites \$GATK_ann/Mills_and_1000G_gold_standard.indels.hg38.vcf  -knownSites \$GATK_ann/Homo_sapiens_assembly38.known_indels.vcf   -o \${PREFIX}.sorted.merged.reorder.rmdup.realign.recal.table -nct 20 &&\\
echo "BR done at `date`" &&\\

\$JAVA -Djava.io.tmpdir=\$TMP -Xmx25g -jar \$GATK -T PrintReads -R \$REF -I \${PREFIX}.sorted.merged.reorder.rmdup.realign.bam -BQSR \${PREFIX}.sorted.merged.reorder.rmdup.realign.recal.table -o \${PREFIX}.sorted.merged.reorder.rmdup.realign.recal.bam -nct 20 && rm \${PREFIX}.sorted.merged.reorder.rmdup.realign.bam &&\\
