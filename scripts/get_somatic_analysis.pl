...
/usr/bin/java -Djava.io.tmpdir=$TMP -jar $GATK -T MuTect2 -R $GENOME -I:tumor $cancer_bam -I:normal $normal_bam --dbsnp $DBSNP --cosmic $COSMIC  -o $outpre.$i.vcf -L $i &&\\
...
EOF
	...
	print OUT<<EOF;
for i in `cat ~/chr.list`; do awk '{if(\$7 == "PASS"){print \$0}}'  $outpre.\$i.vcf;done >> $outpre.PASS.vcf&&\\
...
bam-readcount -w 20 -b 20 -f $GENOME -l $outpre.snv.Somatic.hc.filter.pos $cancer_bam > $outpre.snv.Somatic.hc.filter.rc &&\\
bam-readcount -w 20 -b 20 -f $GENOME -l $outpre.indel.Somatic.hc.pos $cancer_bam > $outpre.indel.Somatic.hc.rc &&\\
...
echo "Mission starts at `date`" &&\\
uname -n &&\\
echo "Call SNV starts at `date`" &&\\
bam-somaticsniper -Q 40 -q 20  -F vcf -G -L -f $GENOME $cancer_bam $normal_bam $outpre.vcf
echo "10col mpileup starts at `date`" &&\\
/lustre/home/acct-medkkw/medlyb/soft/samtools-0.1.16/samtools pileup -cvf $GENOME $cancer_bam > $datadir/$cancer/$cancer.10col.mpileup &&\\
echo "Filter starts at `date`" &&\\
...
bam-readcount -w 20 -b 20 -f $GENOME -l $outpre.vcf.SNPfilter.pos $cancer_bam > $outpre.vcf.SNPfilter.rc &&\\
...
/usr/bin/java -Djava.io.tmpdir=$TMP -jar $GATK -T HaplotypeCaller -R $GENOME -I $cancer_bam --emitRefConfidence GVCF --variant_index_type LINEAR --variant_index_parameter 128000 -o ${cancer}.g.vcf.gz &&\\
echo "Mission completes at `date`"
EOF
	close OUT;
...
/usr/bin/java -Djava.io.tmpdir=$TMP -jar $GATK -T HaplotypeCaller -R $GENOME -I $normal_bam --emitRefConfidence GVCF --variant_index_type LINEAR --variant_index_parameter 128000 -o ${normal}.g.vcf.gz &&\\
echo "Mission completes at `date`"
EOF
	close OUT;
}
